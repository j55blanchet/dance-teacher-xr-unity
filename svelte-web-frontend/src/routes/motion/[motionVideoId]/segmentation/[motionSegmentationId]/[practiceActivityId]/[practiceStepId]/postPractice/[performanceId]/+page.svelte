<script lang="ts">
	...
	onMount(() => {
		...
		$teachingAgent
			.decidePostPracticeAttemptAction({
	...
	async function handleSelfReport(e: SubmitEvent) {
		...
		const selectionLabel = (data.action as PostPracticeAttemptAction_Review).selfReports
		...
		const newAttemptObject = await data.databackend.updateUserPerformanceAttempt(
			data.performanceAttempt.id,
			{
				self_report: {
					prompt: prompt,
	...
	async function executePostPracticeAction(action: PostPracticeAttemptAction) {
		if (action.action === 'immediateRepeat') {
			...
			const currentConsecutiveAttemptNumber =
				data.performanceAttempt?.consecutive_attempt_number ?? 0;
			const nextConsecutiveAttemptNumber = currentConsecutiveAttemptNumber + 1;
			practiceStepUrl.searchParams.set(
				'consecutiveAttemptNumber',
				nextConsecutiveAttemptNumber.toString()
			);
	...
	$effect(() => {
		if (data.action) {
			...
			executePostPracticeAction(data.action).catch((err) => {
...
<div class="p-4">
	<h2 class="mb-4 text-2xl font-bold">{data.practiceActivity.id} {data.practiceStep.id}</h2>
	<p>{displayText}</p>
