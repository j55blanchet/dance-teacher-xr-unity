export default class EventHub<Events extends Record<string, any> = Record<string, any>> {
	 ...
	 */
	subscribe<E extends keyof Events>(event: E, handler: (payload: Events[E]) => void): void {
	 ...
	 */
	unsubscribe<E extends keyof Events>(event: E, handler: (payload: Events[E]) => void): void {
		const handlers = this.listeners[event];
		if (!handlers) return;
