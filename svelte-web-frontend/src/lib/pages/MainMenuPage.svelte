...
let danceTiles = $derived(motionVideos.map(mv => {
    return {
        ...mv,
    ...
    <div class="
        ...
        <div class="daisy-card daisy-card-side bg-base-200 text-base-content shadow-xl" >
            <figure class="aspect-9/16 w-24">
                <img class="h-full w-full aspect-9/16" src={getThumbnailUrl(supabase, tileData.thumbnail_src)} alt={tileData.display_name + " thumbnail"}>
            </figure>
