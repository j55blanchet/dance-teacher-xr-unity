...
export function calculateJointVels<T extends Pose2DPixelLandmarks | Pose3DLandmarkFrame>(
	pose: T,
	pPose: T,
	dt: number,
	scale: number,
 ...
 */
export function calculateJointDerivs(
	cur: VecWithVisibility[],
	prev: VecWithVisibility[],
	dt: number,
	opts?: {
		visibilityBehavior?: 'avg' | 'min';
	}
...
export function calculateKinematicValues<T extends Pose2DPixelLandmarks | Pose3DLandmarkFrame>(
	...
	if (
		matchingUserPoses.length !== referencePoses.length ||
		matchingUserPoses.length !== frameTimes.length
	) {
	...
	function* processFrames() {
		...
		for (let i = 0; i < matchingUserPoses.length; i++) {
			...
			if (
				...
				curPoses.ref[0].x === undefined ||
				curPoses.ref[0].x == null
			) {
			...
			const usrFrameScale = opts?.scaleIndicator
				? typeof opts.scaleIndicator === 'function'
					? opts.scaleIndicator(curPoses.user)
					: (opts.scaleIndicator?.user ?? 1.0)
				: defaultScaleIndicatorFn(curPoses.user as any);

			const refFrameScale = opts?.scaleIndicator
				? typeof opts.scaleIndicator === 'function'
					? opts.scaleIndicator(curPoses.ref)
					: (opts.scaleIndicator?.ref ?? 1.0)
...
export function calculateKinematicErrorDescriptors(
	...
	if (optLandmarkWeights && optLandmarkWeights.length !== numLandmarks) {
		throw new Error(
			`landmarkWeights must be of length ${numLandmarks} (i.e. the number of landmarks)`
		);
	}
	...
	const weighErrorValues = (allFrames: ScalarWithVisibility[][]) => {
		...
		if (visibilityBehavior === 'none') {
			...
			weightedVals = allFrames
				...
				.map((frameValsWeighted) =>
					frameValsWeighted.map((v, landmark_i) => {
						// Divide by the total visibility to rescale the values
						return v / visibilityTotals[landmark_i];
					})
				);
			avgVisibility =
				visibilityTotals.reduce((a, b) => a + b, 0) / (numLandmarks * allFrames.length);
		} else {
		...
		return {
			weightedValues: weightedVals.map((frameVals) => {
				...
				return (
					frameVals.reduce((acc, lmVal, lmIndex) => acc + lmVal * landmarkWeights[lmIndex], 0) /
