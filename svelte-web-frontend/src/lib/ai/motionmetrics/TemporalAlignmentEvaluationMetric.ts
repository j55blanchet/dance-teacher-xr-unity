...
function calculateTemporalAlignmentSignals(history: EvaluationTrackHistory, debugFilepathRoot?: string): TemporalAlignmentSignals {
    ...
    if (debugFilepathRoot) {
        saveTimeSeriesToCSV([userImpactEnvelope, referenceImpactEnvelope], debugFilepathRoot + 'impact_envelopes.csv');
    }
...
export default class TemporalAlignmentEvaluationMetric implements SummaryEvaluationMetric<TemporalAlignmentEvaluationMetricOutput, TemporalAlignmentEvaluationMetricFormattedOutput> {
    summarizeMetric(
    ...
    getTimeSeries(context: EvaluationMetricTimeSeriesContext<TemporalAlignmentEvaluationMetricOutput>): MotionMetricTimeSeries[] {
