...
export type MotionMetricTimeSeries<
	RowType extends MotionMetricTimeSeriesRow = MotionMetricTimeSeriesRow
> = {
	seriesId: string;
	title?: string;
	xKey: string;
...
export type QuantifierTimeSeriesContext<
...
export interface BaseMetric<
	SummaryType,
	FormattedSummaryType extends Record<string, number | string | null>
> {
	formatSummary(summary: Readonly<SummaryType>): FormattedSummaryType;
}

export function createSingleTrackHistory(
	track: Readonly<SingleTrackMetricTrack>
): SingleTrackHistory {
	return {
...
export function getSegmentFrameRanges(
	...
	for (let i = 0; i < normalizedBoundaries.length; i++) {
		...
		if (i > 0 && boundary <= normalizedBoundaries[i - 1]) {
			throw new Error(
				`segment boundaries must be strictly increasing; found ${normalizedBoundaries[i - 1]} then ${boundary}`
			);
		}
	}
...
export function aggregateSegmentedValues(
	...
	if (!ranges) {
		return segmentBoundaries.length === 0
			? [null]
			: new Array(segmentBoundaries.length + 1).fill(null);
	}
...
export interface LiveEvaluationMetric<
	FrameResultType,
	SummaryType,
	FormattedSummaryType extends Record<string, number | string | null>
> extends BaseMetric<SummaryType, FormattedSummaryType> {
	computeMetric(
...
export interface SummaryEvaluationMetric<
	SummaryType,
	FormattedSummaryType extends Record<string, number | string | null>
> extends BaseMetric<SummaryType, FormattedSummaryType> {
	summarizeMetric(history: EvaluationTrackHistory, debugFilepathRoot?: string): SummaryType;
	...
	plotSummary?(element: HTMLElement, summary: SummaryType): Promise<void>;

	getTimeSeries?(context: EvaluationMetricTimeSeriesContext<SummaryType>): MotionMetricTimeSeries[];
}

export interface MotionQuantifierMetric<
	QuantifiedRow extends QuantifierTimeSeriesRow = QuantifierTimeSeriesRow,
	FormattedSummaryType extends Record<string, number | string | null> = Record<
		string,
		number | string | null
	>
> extends BaseMetric<Array<number | null>, FormattedSummaryType> {
	quantify(track: Readonly<SingleTrackMetricTrack>): MotionMetricTimeSeries<QuantifiedRow>[];
