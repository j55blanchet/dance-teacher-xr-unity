...
export default class CorrelationQuantifierMetric {
	...
	quantifyWithBoundaries(
		...
		const rows = getSegmentFrameRanges(track.videoFrameTimesInSecs.length, segmentBoundaries).map(
			(range, segmentIndex) => {
				const segmentValuesA = valuesA
					.filter(
						(entry) =>
							entry.frameIndex >= range.startFrame && entry.frameIndex < range.endFrameExclusive
					)
	...
	formatSummary(summary: Readonly<Array<number | null>>) {
		return Object.fromEntries(
			summary.map((value, index) => [`segment_${index}_correlation`, value])
		);
	}
}
