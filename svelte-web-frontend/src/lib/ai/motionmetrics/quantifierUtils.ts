...
export function reflectPointAcrossLine3D(
	point: { x: number; y: number; z: number },
	lineStart: { x: number; y: number; z: number },
	lineEnd: { x: number; y: number; z: number }
) {
...
export function getSymmetryAxis2D(pose: Readonly<Pose2DPixelLandmarks>) {
	...
	const neckBase = getMidpoint2D(
		pose[PoseLandmarkIds.leftShoulder],
		pose[PoseLandmarkIds.rightShoulder]
	);
...
export function getTrackScale2D(track: Readonly<SingleTrackMetricTrack>) {
	const scales = track.poses2d
		.map((pose) => Get2DScaleIndicator(pose))
...
export function getTrackScale3D(track: Readonly<SingleTrackMetricTrack>) {
	const scales = track.poses3d
		.map((pose) => Get3DScaleIndicator(pose))
		.filter((x) => Number.isFinite(x) && x > 0);
	return scales.length > 0 ? scales.reduce((sum, x) => sum + x, 0) / scales.length : 1;
}

export function getSeriesById<RowType extends QuantifierTimeSeriesRow>(
	seriesCollection: MotionMetricTimeSeries<RowType>[],
	seriesId?: string
) {
...
export function getFrameAlignedRows(series: MotionMetricTimeSeries): QuantifierFrameAlignedRow[] {
	return series.rows.filter(
		(row): row is QuantifierFrameAlignedRow =>
			typeof (row as QuantifierFrameAlignedRow).frameIndex === 'number'
	);
}
...
export function pearsonCorrelation(valuesA: readonly number[], valuesB: readonly number[]) {
	...
	const pairs = valuesA
		.map((value, index) => [value, valuesB[index]] as const)
...
export function getSegmentIndexForFrame(
	frameIndex: number,
	frameCount: number,
	...
	return ranges.findIndex(
		(range) => frameIndex >= range.startFrame && frameIndex < range.endFrameExclusive
	);
}
...
export function getTorsoCenter2D(pose: Readonly<Pose2DPixelLandmarks>) {
	const shoulderCenter = getMidpoint2D(
		pose[PoseLandmarkIds.leftShoulder],
		pose[PoseLandmarkIds.rightShoulder]
	);
	const hipCenter = getMidpoint2D(pose[PoseLandmarkIds.leftHip], pose[PoseLandmarkIds.rightHip]);
...
export function getTorsoCenter3D(pose: Readonly<Pose3DLandmarkFrame>) {
	const shoulderCenter = getMidpoint3D(
		pose[PoseLandmarkIds.leftShoulder],
		pose[PoseLandmarkIds.rightShoulder]
	);
	const hipCenter = getMidpoint3D(pose[PoseLandmarkIds.leftHip], pose[PoseLandmarkIds.rightHip]);
