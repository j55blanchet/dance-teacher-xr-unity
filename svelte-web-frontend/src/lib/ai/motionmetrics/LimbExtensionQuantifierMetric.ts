		...
		const rows = track.videoFrameTimesInSecs.map((videoTimeSecs, frameIndex) => {
			const extensionValues = this.opts.includedLimbs.map((limb) => {
				if (this.opts.dimension === '2d') {
					...
					if (limb === 'leftHand')
						return (
							distance2D(pose[PoseLandmarkIds.leftWrist], getTorsoCenter2D(pose)) /
							Math.max(trackScale, 1e-6)
						);
					if (limb === 'rightHand')
						return (
							distance2D(pose[PoseLandmarkIds.rightWrist], getTorsoCenter2D(pose)) /
							Math.max(trackScale, 1e-6)
						);
					if (limb === 'leftFoot')
						return (
							distance2D(pose[PoseLandmarkIds.leftAnkle], getHipCenter2D(pose)) /
							Math.max(trackScale, 1e-6)
						);
					return (
						distance2D(pose[PoseLandmarkIds.rightAnkle], getHipCenter2D(pose)) /
						Math.max(trackScale, 1e-6)
					);
				}
				...
				if (limb === 'leftHand')
					return (
						distance3D(pose[PoseLandmarkIds.leftWrist], getTorsoCenter3D(pose)) /
						Math.max(trackScale, 1e-6)
					);
				if (limb === 'rightHand')
					return (
						distance3D(pose[PoseLandmarkIds.rightWrist], getTorsoCenter3D(pose)) /
						Math.max(trackScale, 1e-6)
					);
				if (limb === 'leftFoot')
					return (
						distance3D(pose[PoseLandmarkIds.leftAnkle], getHipCenter3D(pose)) /
						Math.max(trackScale, 1e-6)
					);
				return (
					distance3D(pose[PoseLandmarkIds.rightAnkle], getHipCenter3D(pose)) /
					Math.max(trackScale, 1e-6)
				);
			});
