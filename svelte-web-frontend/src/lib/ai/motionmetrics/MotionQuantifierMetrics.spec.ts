...
class FakeFrameSeriesMetric extends FrameAlignedMotionQuantifierMetric {
    constructor(private values: number[], private valueKey = "value", private seriesId = "fake") {
        super();
    }
...
describe("Motion quantifier metrics", () => {
    it("body symmetry is higher for symmetric 2D/3D poses", ({ expect }) => {
        ...
        const track2d = createTrackFromFrames([{ timeSecs: 0, pose2d: sym2d, pose3d: sym3d }, { timeSecs: 1, pose2d: asym2d, pose3d: asym3d }]);
    ...
    it("extrema window suppression keeps stronger extrema only", ({ expect }) => {
        const track = createTrackFromFrames(new Array(7).fill(0).map((_, i) => ({ timeSecs: i, pose2d: createEmptyPose2D(), pose3d: createEmptyPose3D() })));
    ...
    it("correlation returns positive, negative, and null cases", ({ expect }) => {
        const track = createTrackFromFrames(new Array(6).fill(0).map((_, i) => ({ timeSecs: i, pose2d: createEmptyPose2D(), pose3d: createEmptyPose3D() })));
        ...
        expect(negativeMetric.quantify(createTrackFromFrames(new Array(4).fill(0).map((_, i) => ({ timeSecs: i })) as any))[0].rows[0].correlation).toBeCloseTo(-1);
        expect(nullMetric.quantify(createTrackFromFrames(new Array(3).fill(0).map((_, i) => ({ timeSecs: i })) as any))[0].rows[0].correlation).toBeNull();
    });
