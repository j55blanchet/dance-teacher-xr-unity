...
export default class ExtremaQuantifierMetric extends DerivedSeriesMotionQuantifierMetric {
	...
	quantify(
	...
	): MotionMetricTimeSeries<QuantifierDerivedRow>[] {
		const sourceSeries = getSeriesById(
			this.opts.sourceMetric.quantify(track),
			this.opts.sourceSeriesId
		);
		...
		for (const mode of modes) {
			for (let index = 1; index < values.length - 1; index++) {
				...
				const isLocal =
					mode === 'min' ? current <= prev && current <= next : current >= prev && current >= next;
				if (
					!isLocal ||
					!this.isSelectedExtremum(
	...
	getSegmentationBoundaries(track: Readonly<SingleTrackMetricTrack>) {
		...
		return this.quantify(track)[0]
			.rows.map((row) => row.sourceFrameStart)
	...
	formatSummary(summary: Readonly<Array<number | null>>) {
		return Object.fromEntries(
			summary.map((value, index) => [`segment_${index}_extremaCount`, value])
		);
	}
}
