...
export function getFixtureClipId(identity: StudyMetricFixtureIdentity) {
    return [
        ...
        identity.workflowId,
    ].map(sanitizeArtifactSegment).join("__");
}

export function buildTrackHistoryForStudyClip(segmentData: StudySegmentData, referencePoses: PoseFrame[]): EvaluationTrackHistory {
    const fps = 30;
    ...
    return {
        ...
        actualTimesInMs: referencePoses.map((_, i) => i / (segmentData.segmentInfo.performanceSpeed * fps)).slice(0, shortestClipLength),
        ...
        user3DFrameHistory: segmentData.poses.map((pose) => pose.worldPose).slice(0, shortestClipLength),
        user2DFrameHistory: segmentData.poses.map((pose) => pose.pixelPose).slice(0, shortestClipLength),
    };
}

export function buildTestTrackForStudyClip(segmentData: StudySegmentData, referencePoses: PoseFrame[]): TestTrack {
    const fps = 30;
    const shortestClipLength = Math.min(segmentData.poses.length, referencePoses.length);
...
export type StudyMetricFixtureSelector = Pick<StudyMetricFixtureIdentity, "study" | "userId" | "danceName" | "workflowId" | "clipNumber">;

export function doesFixtureMatchSelection(fixture: StudyMetricFixture, selection: StudyMetricFixtureSelector) {
    return fixture.identity.study === selection.study &&
