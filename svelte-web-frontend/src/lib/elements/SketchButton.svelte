<script lang="ts">
export let disabled: boolean = false
</script>

<button on:click disabled={disabled} >
    <span>
        <slot></slot>
    </span>
</button>

<style>
button span {
    background: #FFF;
    display: block;
    padding: calc(5em / 16) calc(15em / 16);
    /* border-radius: calc(5em / 16); */
    border-radius: 255px 15px 225px 15px/15px 225px 15px 255px;
    border: calc(2em / 16) solid #000;
}
button:hover span {
    background: #EEE;
}
button:hover {
    box-shadow: 0 0.125em 0 #000, 0px 0.125em 0.125em 0.125em var(--color-theme-1);
}
button:active span {
    background: #DDD;
}
button:active {
    top: 0.25em;
    padding-bottom: 0px;
    box-shadow: 0 0.0625em 0 #000;
}
button {
    cursor: pointer;
    position: relative;
    /* float: left; */
    font: normal 1em "Patrick Hand", sans-serif;
    /* margin-right: 0.625em; */
    text-transform: lowercase;
    color: #000;
    text-decoration: none;
    padding: 0;
    padding-bottom: calc(3em / 16);
    border: 0;
    /* border-radius: calc(5em / 16); */
    border-radius: 255px 15px 225px 15px/15px 225px 15px 255px;
    box-shadow: 0 0.125em 0 #000;
    transition: padding 0.1s, box-shadow 0.1s, top 0.1s, background 0.1s;
    background-image: url('data:image/gif;base64,');
}
</style>