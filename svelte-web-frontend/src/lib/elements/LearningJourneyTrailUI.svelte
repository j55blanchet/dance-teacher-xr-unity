<script lang="ts">
	...
	$: isVeryFirstActivityStep =
		practicePlanProgress === undefined ||
		Object.values(practicePlanProgress).reduce(
			(nonCompleteSoFar, activityProgress) =>
				nonCompleteSoFar &&
				Object.values(activityProgress).reduce(
	...
	{#each practicePlan.stages as stage}
		<div class="relative flex flex-row flex-wrap justify-center gap-4">
			{#each stage.activities as activity}
				...
				{@const isSuggestingFirstStepInActivity =
					isActivitySuggested &&
					activitySuggestedStepId &&
					activitySuggestedStepId === activity.steps[0]?.id}
				{@const isSuggestingLastStepInActivity =
					isActivitySuggested &&
					activitySuggestedStepId &&
					activitySuggestedStepId === activity.steps[activity.steps.length - 1]?.id}
				{@const activityTooltipText = !isActivitySuggested
					? undefined
					: isVeryFirstActivityStep
						? 'Start learning here!'
						: isSuggestingFirstStepInActivity
							? "Let's start this activity!"
							: isSuggestingLastStepInActivity
								? "Let's finish this activity!"
								: 'Continue with this activity!'}

				<div class="flex flex-col items-center justify-start">
					<details class="daisy-dropdown" bind:open={openActivityDropdowns[activity.id]}>
						<summary
							tabindex="0"
							class="daisy-btn flex size-32 items-center
						...
						>
							{#if activity.type === 'segment'}
								<span class="rounded-full bg-green-500 px-8 py-1">
							...
							<div class="daisy-card-body p-0">
								...
								<PracticeStepList
									...
									on:practiceStepClicked={(e) =>
										onPracticeStepClicked(e.detail.activity, e.detail.step)}
