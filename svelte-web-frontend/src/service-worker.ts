...
const idb: AsyncStorage = {
    ...
    async setItem(k, v) { const c = await caches.open('sb-auth'); await c.put(new Request('https://auth/' + k), new Response(v)); },
    async removeItem(k) { const c = await caches.open('sb-auth'); await c.delete('https://auth/' + k); }
};
...
self.addEventListener('message', (e: any) => {
    ...
    if (msg?.type === 'SUPABASE_SESSION') {
        ...
        e.waitUntil(supabase.auth.setSession({
