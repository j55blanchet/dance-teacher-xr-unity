    ...
    for i, filepath in enumerate(filepaths):
        ...
        if filepath.suffix in ACCEPT_VIDEO_FILES:
            ...
            if cached_audio_filepath:
                ...
                save_audio_from_video(filepath, audio_filepath)
                print(f'Done (saved to {audio_filepath.relative_to(args.audiocachedir)})')


        # Load the audio file
        audio_array, sample_rate = load_audio(audio_filepath)

        # Calculate the tempo information
        tempo_info: TempoInfo = calculate_tempo_info(audio_array, sample_rate)

        samples_per_beat = sample_rate * 60 / tempo_info.bpm
        starting_beat_sample = int((tempo_info.starting_beat_offset * sample_rate) % samples_per_beat)
        ...
        eight_beat_segments_as_timestamps = np.array(eight_beat_segments) / sample_rate
        ...
        analysis_result = {
            ...
            'eight_beat_segments': eight_beat_segments_as_timestamps.tolist(), # Convert to list to make it JSON serializable
